...
`#name Hide Things - 4.Restore-Auto`;

`#description Paste the Hide Things - 1.Strip data into the "data" option below, then run. Puts every stripped thing back at its original index with type, position, facing and all flags. Things currently in the map are left as they are.`;

`#scriptoptions

data
{
    description = "Hide Things - 1.Strip data (paste the log contents)";
    default = "";
    type = 2;
}
`;
...
// ---------------------------------------------------------------------------
// Hide Things - 4.Restore-Auto - MBF21
// Same as Hide Things - 2.Restore, but reads the data from the docker option.
...
// FNV-1a 32-bit hash - must match Hide Things - 1.Strip
...
const trRaw = String(UDB.ScriptOptions.data || '').replace(/\s+/g, '').toLowerCase();

if (trRaw.length === 0)
    UDB.die('The "data" script option is empty. Paste the Hide Things - 1.Strip string into it first. Map not changed.');
